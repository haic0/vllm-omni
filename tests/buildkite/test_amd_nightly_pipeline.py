...
def test_nightly_inventory_is_explicit_and_non_blocking() -> None:
    steps = _leaf_steps()
    assert {step["label"] for step in steps} == EXPECTED_LABELS
    assert all(step["grade"] == "NonBlocking" for step in steps)
    assert all(step["depends_on"] == "image-build" for step in steps)
...
def test_accuracy_results_are_published_as_artifacts() -> None:
    accuracy = next(
        step for step in _leaf_steps() if step["label"] == "Qwen3-Omni Accuracy"
    )
