...
def _compute_bootstrap_if_exprs(*, decision, platform: str) -> dict[str, str]:
    ...
    if platform == "amd":
        if decision.skip_all:
            ...
            amd_parts = [f"({NIGHTLY_LABEL_IF})"]
            if decision.is_run("amd", "l2"):
                # The AMD selector preserves ready for admitted, unlabeled PRs.
                amd_parts.append('(build.branch != "main")')
            if decision.is_run("amd", "l3"):
                amd_parts.append(f"({MERGE_UPLOAD_IF})")
            amd_expr = " || ".join(amd_parts)
        else:
            amd_expr = BOOTSTRAP_ENABLED_IF
        formatted_amd_expr = _format_bootstrap_if(amd_expr)
        expressions["image"] = formatted_amd_expr
        expressions["amd"] = formatted_amd_expr
    return expressions
...
def _select_amd_suites() -> tuple[str, ...]:
    ...
    labels = () if debug_test_yaml.strip() else _amd_pr_labels()
    return select_amd_test_suites(
